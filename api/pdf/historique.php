...
$clientp->name=$historique->cliname;
 class PDFHistorique extends FPDF{
    ...
    function header(){
        ...
        $this->Cell(195,2,'Historique d\'une Voiture',0,0,'C');
        $this->Ln(9);      


        $this->SetFont('Times','',12);
        $this->Cell(190,2,$this->historique->voiture_imma.', Le : '.$this->historique->created_at,0,0,'C');
        $this->Ln(20);
            ...
            if(!empty($this->imagps)){
                if(count($this->fams)<4){
                    ...
                    $this->Ln();
                        ...
                        $this->Cell(190,35,'',1,1,'C');
                            if(!empty($this->imagps[0])){
                                $this->image("../voiture/images/".$this->imagps[0],13,211,0,26);
                            }
                            if(!empty($this->imagps[1])){
                                $this->image("../voiture/images/".$this->imagps[1],63,211,0,26);
                            }
                            if(!empty($this->imagps[2])){
                                $this->image("../voiture/images/".$this->imagps[2],113,211,0,26);
                            }
                            if(!empty($this->imagps[3])){
                                $this->image("../voiture/images/".$this->imagps[3],163,211,0,26);
                            }
                    ...
                    $this->Ln();
                        ...
                        $this->Cell(190,35,'',1,1,'C');
                            if(!empty($this->imagps[0])){
                                $this->image("../voiture/images/".$this->imagps[0],13,225,0,26);
                            }
                            if(!empty($this->imagps[1])){
                                $this->image("../voiture/images/".$this->imagps[1],63,225,0,26);
                            }
                            if(!empty($this->imagps[2])){
                                $this->image("../voiture/images/".$this->imagps[2],113,225,0,26);
                            }
                            if(!empty($this->imagps[3])){
                                $this->image("../voiture/images/".$this->imagps[3],163,225,0,26);
                            }
                    ...
                    $this->Ln();
                        ...
                        // $this->Cell(190,35,'',1,1,'C');
                            if(!empty($this->imagps[0])){
                                $this->image("../voiture/images/".$this->imagps[0],13,230,0,26);
                            }
                            if(!empty($this->imagps[1])){
                                $this->image("../voiture/images/".$this->imagps[1],63,230,0,26);
                            }
                            if(!empty($this->imagps[2])){
                                $this->image("../voiture/images/".$this->imagps[2],113,230,0,26);
                            }
                            if(!empty($this->imagps[3])){
                                $this->image("../voiture/images/".$this->imagps[3],163,230,0,26);
                            }
                }
...
$pdf->demandeClient();
$pdf->ImagesR();
$pdf->Output("D","Historique_du_voiture_".$historique->voiture_imma.".pdf");
